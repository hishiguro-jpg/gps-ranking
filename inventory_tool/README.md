...
| units_per_pallet | 1パレットあたりの個数（パレット単位で発注しない場合は0） | 36 |

※ `fixed_cost`は「上記以外の固定費」を入力する。保管費用（現在庫数×単価）や搬入トラック代は
　`--holding-cost-per-unit`・`--truck-cost`・`--pallets-per-truck`で指定すれば自動的に加算される
　（`units_per_pallet`を指定した商品のみ、推奨発注量もパレット単位で切り上げられる）。
...
| `--pallets-per-truck` | 16 | トラック1台に積めるパレット数 |
| `--truck-cost` | 0 | トラック1台あたりの搬入費用(円)。0なら考慮しない |
| `--holding-cost-per-unit` | 0 | 現在庫1個あたりの保管費用(円)。固定費に自動加算 |
...
- **推奨発注量(切り上げ前)**：`平均消化ペース × (リードタイム日数 + 目標日数) + 安全在庫 − 現在庫 − 発注済み数量`（0未満は0）
- **推奨発注量(最終)**：`units_per_pallet`を指定した商品は、上記をパレット単位で切り上げ
- **固定費(合計)**：`cost_master.csvのfixed_cost + 現在庫×holding-cost-per-unit + 搬入トラック台数×truck-cost`
  （トラック台数 = 発注パレット数 ÷ `pallets-per-truck` の切り上げ）
- **損益分岐点**：`固定費(合計) ÷ (売価 − 原価)` で必要販売数量を算出し、現在の消化ペースで到達日を見積もり
- **製造費回収(資金繰りの目安)**：`原価 × (現在庫+推奨発注量) ÷ (売価 × 平均消化ペース)` で日数を算出。
  原価は売れるたびに全額戻ってくる前提のため、固定費(トラック代・保管費等)の回収とは別の指標として扱う
  （原価＝売価で商品取引が損益ゼロの業態で、「製造費を回収した後は実質すべて利益」という社内基準がある場合に使う）
